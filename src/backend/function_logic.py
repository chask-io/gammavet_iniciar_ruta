"""Business logic for the dedicated Gammavet route-start lambda."""
...
class FunctionBackend:
    """Start or claim the exact intended route stop for a Gammavet driver."""
    ...
    def __init__(self, orchestration_event: OrchestrationEvent):
        ...
        logger.info(
            "Initialized IniciarRutaFn for org=%s",
    ...
    def process_request(self) -> str:
        return _start_requested_stop(
            ...
            message_prefix="Retiro de muestras",
            started_label="iniciada",
        )
...
def _start_requested_stop(
    ...
    *,
    ...
    if not claimed and stop is None:
        ...
        return "No hay paradas asignadas para el conductor. Mensaje enviado."
    ...
    if requested_route_stop_id and returned_stop_id and str(returned_stop_id) == str(
        ...
        return (
            "No-op: la ruta ya estaba en curso en la misma parada solicitada. "
            "No se envio un nuevo WhatsApp al conductor."
        )
    ...
    if requested_route_stop_id and str(returned_stop_id) != str(requested_route_stop_id):
        ...
        return (
            "Bloqueada respuesta de ruta por mismatch de route_stop_id. "
            "Mensaje seguro de aclaracion enviado al conductor."
        )
